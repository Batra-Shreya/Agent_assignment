...
export default function WorkflowsPage() {
  ...
  const handleExecute = async () => {
    if (!activeWorkflowId || !userPrompt.trim()) {
      alert("Please load a workflow and enter a prompt.");
      return;
    }
    ...
    try {
      ...
      const data = await res.json();
      if (res.ok) {
        setExecutionResult(data.result || "Execution complete.");
      } else {
        setExecutionResult(`Error: ${data.detail || "Execution failed"}`);
      }
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="flex h-screen bg-gray-50">
      <div className="w-64 bg-white border-r p-6 flex flex-col gap-6 shrink-0 overflow-y-auto">
        ...
        <div className="flex flex-col gap-3">
          <label className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Enter Prompt</label>
          <textarea
            value={userPrompt}
          ...
          <button
            ...
            disabled={isExecuting || !activeWorkflowId || !userPrompt.trim()}
            className="flex items-center justify-center gap-2 bg-green-600 text-white px-4 py-2 rounded-lg hover:bg-green-700 disabled:opacity-40 disabled:cursor-not-allowed transition font-medium text-sm"
          >
            {isExecuting ? "Executing..." : "Execute"}
          </button>
          {executionResult && (
            <div className="bg-gray-100 rounded-lg px-3 py-2 text-xs font-mono text-gray-700 max-h-24 overflow-y-auto">
              {executionResult}
            </div>
          )}
        </div>
